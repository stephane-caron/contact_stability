...
from pymanoid import Contact, ContactSet, Cube, draw_2d_cone, get_viewer
...
def compute_support_area(contacts):
    global area_handle, area_handle1
    G = contacts.compute_grasp_matrix([0, 0, 0])
    F = -contacts.compute_stacked_wrench_cones()
    try:
        vertices1, rays1 = compute_pendular_area_bretl(
            G, F, mass, com.p, z_out)
        area_handle1 = draw_2d_cone(
            vertices1, rays1, numpy.array([0, 0, 1]), 'b-#')
        vertices, rays = compute_pendular_area_cdd(
            G, F, mass, com.p, z_out)
    ...
    contacts = ContactSet([
        Contact(
            ...
            pos=[(2 * i - 1) * numpy.random.random(),
                 ...
                 0],
            rpy=0.1 * numpy.random.random(3),
            friction=0.5,
            visible=True
        )
        for i in xrange(nb_contacts)])
    com = Cube(0.05, [0., 0., 1.], color='b')
